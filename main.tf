...
# ################################################################################
# # New Karpenter Node Pool with Taints
# ################################################################################


# resource "kubectl_manifest" "karpenter_node_pool_specific" {
#   yaml_body = <<-YAML
#     apiVersion: karpenter.sh/v1beta1
#     kind: NodePool
#     metadata:
...
module "eks" {
  ...
  vpc_id                   = "vpc-031be88caa05f41a7"
  subnet_ids               = ["subnet-0cdf1ca48ccd1f647","subnet-08564fc17f885cb62"]
